...
collections = [db['all_dims_amazon_dataset']]

indexes = ['all_dims_vector_index']
...
filename = "exact_results_data_voyage_2048.json"
...
for i_0, coll in enumerate(collections):
	for test in tests:
		print(f"running exact test 5.4 {test}\n")
			...
			for i_1, k in enumerate(k_values): 
				for multiplier in mult:
					...
					for query_test_size in query_test_sizes:
						...
						plot_results[f"5.5M.{dimension}.k{k}.candidates{candidates}.{test}.concurrency{concurrency}.test_queries{query_test_size}"] = {
...
df.to_csv(f'results/5_2025_runs/5M_s30_m20_amazon_ecommerce_all_runs_voyage_multidim_scalar.csv')
